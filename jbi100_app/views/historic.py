...
class Historic(html.Div):
    ...
    def build_historic(self, team1, team2):
        ...
        #Team 1 home scores
        home_scores_by_year_team1 = df[df['home_team'] == team1][['Year', 'home_score', 'away_score', 'away_team', 'home_team']]
        home_scores_by_year_team1['is_winner'] = home_scores_by_year_team1['home_score'] > home_scores_by_year_team1['away_score']
        home_scores_by_year_team1 = home_scores_by_year_team1.drop(columns=['away_score'])

        #Team 2 home scores
        home_scores_by_year_team2 = df[df['home_team'] == team2][['Year', 'home_score', 'away_score', 'away_team', 'home_team']]
        home_scores_by_year_team2['is_winner'] = home_scores_by_year_team2['home_score'] > home_scores_by_year_team2['away_score']
        home_scores_by_year_team2 = home_scores_by_year_team2.drop(columns=['away_score'])

        #Team 1 away scores
        away_scores_by_year_team1 = df[df['away_team'] == team1][['Year', 'away_score', 'home_score', 'home_team', 'away_team']]
        away_scores_by_year_team1['is_winner'] = away_scores_by_year_team1['away_score'] > away_scores_by_year_team1['home_score']
        away_scores_by_year_team1 = away_scores_by_year_team1.drop(columns=['home_score'])

        #Team 2 away scores
        away_scores_by_year_team2 = df[df['away_team'] == team2][['Year', 'away_score', 'home_score', 'home_team', 'away_team']]
        away_scores_by_year_team2['is_winner'] = away_scores_by_year_team2['away_score'] > away_scores_by_year_team2['home_score']
        away_scores_by_year_team2 = away_scores_by_year_team2.drop(columns=['home_score'])

        #Concatenate home and away scores for each team
        scores_by_year_team1 = pd.concat([home_scores_by_year_team1, away_scores_by_year_team1])
        scores_by_year_team1 = scores_by_year_team1.sort_values('Year')

        scores_by_year_team2 = pd.concat([home_scores_by_year_team2, away_scores_by_year_team2])
        scores_by_year_team2 = scores_by_year_team2.sort_values('Year')
        
        unique_years = list(set(scores_by_year_team1['Year']).union(set(scores_by_year_team2['Year'])))
        unique_years.sort()
        ...
        def get_color(is_home_team, is_home,is_winner):
            ...
            if is_winner:
                adjusted_color = 'rgba({}, {}, {}, {})'.format(int(color[0]), int(color[1]), int(color[2]), 1)
            else:
                adjusted_color = 'rgba({}, {}, {}, {})'.format(int(color[0]), int(color[1]), int(color[2]), 0.5)
        ...
        for index, row in home_scores_by_year_team1.iterrows():
            ...
            self.figure.add_trace(go.Bar(
                ...
                hoverinfo='y+text',
                hovertext= 'Won Against ' + str(row['away_team']) if row['is_winner'] else 'Lost Against ' + str(row['away_team']),
                showlegend=show_legend,
                legendgroup= 'Home Scores Team 1',
                xaxis=f'x{year_index}'
            ))
        ...
        for index, row in away_scores_by_year_team1.iterrows():
            ...
            self.figure.add_trace(go.Bar(
                ...
                hoverinfo='y+text',
                hovertext= 'Won Against ' + str(row['home_team']) if row['is_winner'] else 'Lost Against ' + str(row['home_team']),
                showlegend=show_legend,
        ...
        xaxes = {
            f'xaxis{i+1}': dict(
                domain=[i*domain_size, (i+1)*domain_size],
                anchor=f'x{i+1}',
        ...
        # Annotation for each year
        annotations = [
            dict(
                x=i*domain_size + domain_size/2,  # x position of the annotation (center of the domain)
                y=0,  # y position of the annotation (below the x-axis)
                text=str(year),  # annotation text (year)
        ...
        annotations.append(
            dict(
                ...
                y=-0.055,  # y position of the annotation (below the tick labels)
                text='Year',  # annotation text
                showarrow=False,  # hide the arrow
                xref='paper',  # x position is relative to the width of the plot
                yref='paper',  # y position is relative to the height of the plot
        ...
        self.figure.update_layout(
            ...
            margin=dict(l=50, r=50, t=50, b=50),  # Adjust the values as needed for padding
